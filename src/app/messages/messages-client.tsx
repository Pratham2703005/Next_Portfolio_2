...
export function MessagesClient({ initialMessages, isAdmin, userEmail, userData }: MessagesClientProps) {
  ...
  const messagesUrl = isAdmin 
    ...
    : `/api/messages/forUsers?email=${encodeURIComponent(userEmail)}`
    
  const { data: messages = initialMessages, error } = useSWR(
    messagesUrl,
    fetcher,
    {
